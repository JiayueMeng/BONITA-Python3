...
def buildToolbox(individualLength, bitFlipProb, model, params):
    ...
    )  # create a class of individuals that are lists
    ...
    toolbox.register(
        "Individual", tools.initIterate, creator.Individual, toolbox.genRandomBitString
    )
...
def evaluateByNode(individual, cells, model, sss, params, KOlist, KIlist, boolC):
    ...
    return tuple(
        [
            numpy.sum(
                [
                    (boolValues[j][i] - sss[j][model.nodeList[i]]) ** 2
...
def runProbabilityBooleanSims(
    individual, model, sampleNum, cells, params, KOlist, KIlist, boolC
):
    seeds = []
    ...
    samples = [
        sampler(
            individual, model, sampleNum, seeds[i], params, KOlist[i], KIlist[i], boolC
        )
...
def varOrAdaptive(population, toolbox, model, lambda_, cxpb, mutpb, genfrac, mutModel):
    ...
    for _ in range(lambda_):
        ...
        if op_choice < cxpb:  # Apply crossover
            ind1, ind2 = list(map(toolbox.clone, sample(population, 2)))
...
def selectMutNode(errors):
    normerrors = [
        1.0 * error / numpy.sum(errors) for error in errors
...
def mutFlipBitAdapt(indyIn, genfrac, mutModel):
    ...
    if numpy.sum(errors) < 0.05 * errorNodes or errorNodes == 0:
        ...
        if len(model.possibilityList[focusNode]) > 3 and random() < mutModel:
            ...
            while len(temppermup) < 3:
                ...
                if tempsum == 0:
                    ...
                    recalc = numpy.cumsum([1.0 * rval / tempsum for rval in rvals])
...
def eaMuPlusLambdaAdaptive(
    ...
    fitnesses = [
        evaluateByNode(
            indy[1], params.cells, indy[0], newSSS, params, KOlist, KIlist, boolC
        )
    ...
    for ind in population:
        if numpy.sum(ind.fitness.values) < 0.01 * len(ind.fitness.values):
            breaker = True
    if breaker:
    ...
    for gen in range(1, ngen + 1):
        offspring = varOrAdaptive(
            population,
            toolbox,
            model,
            lambda_,
            0.5 + 0.5 * (1.0 - 1.0 * gen / ngen),
            (0.5 * gen / ngen),
            (1.0 * gen / ngen),
            mutModel,
        )
...
def GAsearchModel(model, sampleList, params, KOlist, KIlist, namer, boolC):
    ...
    toolbox, stats = buildToolbox(
        model.size, params.bitFlipProb, model, params
    )  # set up toolbox
    # run GA, find best in population, return
    population, logbook = eaMuPlusLambdaAdaptive(
        toolbox,
        model,
        mu=params.mu,
...
def localSearch(model, indy, newSSS, params, KOlist, KIlist, boolC):
    outputs = [
        checkNodePossibilities(
            node, indy, newSSS, params.cells, model, params, KOlist, KIlist, boolC
        )
...
def checkNodePossibilities(
    node, indy, newSSS, cellNum, model, params, KOlist, KIlist, boolC
):
    tol = 0.01 * len(newSSS)  # set tolerance for equivalence
    ...
    for i in range(1, 2 ** (end - start)):
        ...
        currentsumtemp = evaluateByNode(
            tempultimate, cellNum, model, newSSS, params, KOlist, KIlist, boolC
        )
