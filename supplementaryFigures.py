        ...
        if len(preds_n1) >= 1:
            ...
            for pred1 in preds_n1:
                for pred2 in preds_n1:
                    if pred1 != pred2:
                        temp1 = set(
                            list(nx.dfs_predecessors(graph, pred1).keys())
                            + list(nx.dfs_predecessors(graph, pred1).values())
                        )
                        temp2 = set(
                            list(nx.dfs_predecessors(graph, pred2).keys())
                            + list(nx.dfs_predecessors(graph, pred2).values())
                        )
...
def scoreERS(codes, iterations=25):
    ...
    for code in codes:
        ...
        )  # get around the problem of disconnected graphs
        ...
        if len(graph) >= 3:
            ...
            for iteration in range(1, iterations + 1):
                ...
                pickleFile = str(
                    "pickles/" + code + "_" + str(iteration) + "_local1.pickle"
                )
                ...
                for node in range(0, len(model1.nodeList)):
                    ...
                    start1, end1 = findEnds2(
                        model1, model1.nodeList[node], equivalents[node]
                    )
...
def makeRNdataframe(scoreERS_output, ifPrint=1, filename="scoreERS.csv"):
    ...
    allRes_df = pd.DataFrame(iter(allRes_flat.items()))

    allRes_df[
        ["ruleLengths", "rnAllNodes", "ImportanceVals", "inDegree", "scoreFunction6"]
    ] = pd.DataFrame(
    ...
    allRes_df[["Pathway", "Iteration", "Node"]] = pd.DataFrame(
        [x[0].split(":", 2) for x in allRes_df[[0]].values], index=allRes_df.index
    )

    allRes_df[
        ["ruleLengths", "ImportanceVals", "inDegree", "scoreFunction6"]
    ] = allRes_df[
        ["ruleLengths", "ImportanceVals", "inDegree", "scoreFunction6"]
    ].apply(
...
def Fig2PanelD(allRes_df):
    ...
    plt.ylim(bottom=0)
    finishPlot(
        "Log2(Total Ancestor Overlap)", "Log2(Length of ERS)", "Fig2_PanelD.png"
...
def supplementary1(allRes_df):
    ...
    plt.legend(bbox_to_anchor=(1.1, 0.8), bbox_transform=plt.gcf().transFigure)
    plt.xlim(1, 7)
    sns.set_style("ticks")
    plt.rc("font", size=8)  # controls default text sizes
    plt.rc("axes", titlesize=8)  # fontsize of the axes title
    ...
    plt.rc("figure", titlesize=8)  # fontsize of the figure title
    finishPlot(
        "Rule Number", "Frequency", "supplementary1.png"
    )  # finishPlot(xlabeler, ylabeler, plotname)
                ...
                for node in range(0, len(model1.nodeList)):
                    ...
                    if model1.nodeList[node] in list(
                        ...
                        allRes[str(code)][str(iteration)][
                            ...
                            authorities[model1.nodeList[node]],
                            inDegree,
...
def supplement_S3(pathwayGraphs):
    ...
    print(("Number of test networks: ", len(list(pathwayGraphs.keys()))))
    fh = open("testSimParams.csv", "w+")
    fh.write(
        "".join(
            [
                "Pathway_ID,",
                "Length_of_shortest_longest_path,",
                "Number_of_nodes_in_pathway,",
                "Number_of_source_nodes,",
                "Mean_out_degree_of_source_nodes",
                "\n",
            ]
        )
    )
    ...
    for name, path in list(pathwayGraphs.items()):
        print((name, path))
        print(validNets)
        if nx.number_of_nodes(path) == 0:
            ...
            for node, indeg in list(inDegree.items()):
                if indeg == 0:
...
def pipeline_fig_s3(filename):
    ...
    nc.parseKEGGdicthsa(
        "inputData/hsa00001.keg", aliasDict, dict1
    ...
    namelist = list(keggDict.keys())  # retain all pathways
    pathwayGraphs = {}  # dictionary of digraphs
    for name in namelist:
        pathwayGraphs[name] = retrieveGraph(
            name, aliasDict, dict1, dict2
        )  # find and store gpickles for graphs found
    pickle.dump(
        pathwayGraphs, open("pathwayGraphs.pickle", "wb")
...
def makeHist(resFile):
    ...
    temp1 = list(
        filter(
            filterInts, list(resFile.loc[:, "Length_of_shortest_longest_path"].values)
        )
    )
    ...
    ax = sns.distplot(
        temp1, kde=False, hist_kws={"alpha": 1, "color": "grey"}, bins=my_bins
    )
    plt.legend(bbox_to_anchor=(1.1, 0.8), bbox_transform=plt.gcf().transFigure)
    plt.tight_layout()
    ...
    ax = ax.get_figure()
    finishPlot(
        "Length of longest shortest path", "Frequency", "keggCycles.png"
    )  # finishPlot(xlabeler, ylabeler, plotname)
    plt.close()
...
def printStats(resFile):
    ...
    statDict["validNets"] = len(
        list(
            filter(
                filterInts,
                list(resFile.loc[:, "Length_of_shortest_longest_path"].values),
            )
        )
    )
    statDict["meanLength"] = np.mean(
        np.array(
            [
                int(t)
                for t in filter(
    ...
    statDict["maxNetSize"] = max(
        [
            int(t)
            for t in filter(
                filterInts, list(resFile.loc[:, "Number_of_nodes_in_pathway"].values)
            )
        ]
    )
    statDict["minNetSize"] = min(
        [
            int(t)
            for t in filter(
                filterInts, list(resFile.loc[:, "Number_of_nodes_in_pathway"].values)
    ...
    statDict["meanOutDegreeOfSource"] = np.mean(
        np.array(
            [
                float(t)
                for t in filter(
                    filterInts,
                    list(resFile.loc[:, "Mean_out_degree_of_source_nodes"].values),
                )
            ]
        )
    )
...
def makeRNplots(codes, iterations=25):
    ...
    allRes_df = allRes_df["ruleLengths", "ImportanceVals", "scoreFunction6"].agg(
        np.mean
    )
    Fig2PanelD(
        allRes_df
    )  # Make plot corresponding to Fig2 Panel D of main text - The log average total size of ERS for each node are plotted against the log total ancestor overlap which is the sum of the pairwise shared ancestor number between any two upstream nodes
