        ...
        if (len(preds_n1) >= 1):
            ...
            for pred1 in preds_n1:
                for pred2 in preds_n1:
                    if pred1 != pred2:
                        temp1=set(list(nx.dfs_predecessors(graph, pred1).keys()) + list(nx.dfs_predecessors(graph, pred1).values()))
                        temp2=set(list(nx.dfs_predecessors(graph, pred2).keys()) + list(nx.dfs_predecessors(graph, pred2).values()))
...
def makeHist(resFile):
    ...
    sns.set_palette(sns.color_palette("Greys_r", 6))
    temp1=list(filter(filterInts, list(resFile.loc[:, 'Length_of_shortest_longest_path'].values)))
...
def printStats(resFile):
    ...
    statDict["validNets"]=len(list(filter(filterInts, list(resFile.loc[:, 'Length_of_shortest_longest_path'].values))))
