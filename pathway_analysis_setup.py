	...
	for entry in soup.find_all('entry'):
		...
		if len(entry_split)>2:
			if entry_split[0]==organism or entry_split[0]=='ko':
				...
				namer=entry_split.pop(0)
				namer=namer.split()[0]
				entry_name=entry_name+useDict[namer] if namer in list(useDict.keys()) else entry_name+namer
			...
			if entry_split[0]==organism:
				...
				entry_type = entry['type']
				entry_name = orgDict[entry_name] if entry_name in list(orgDict.keys()) else entry_name
			elif entry_split[0]=='ko':
				entry_name=entry_split[1]
				entry_type = entry['type']
				entry_name = KEGGdict[entry_name] if entry_name in list(KEGGdict.keys()) else entry_name
