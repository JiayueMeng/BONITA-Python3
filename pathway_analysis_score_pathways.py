...
def outputGraphs(pathway, RAval, comparator, pathImportances):
    ...
    nx.set_node_attributes(original,values={k: k for k in list(original.nodes())},name="Display Name")
    nx.set_node_attributes(original, name = "andNode", values = {k: 0 for k in list(original.nodes())})
    nx.set_node_attributes(
        original, name = "RA", values = {k: RAval[k] for k in list(original.nodes())}
    )
    ...
    nx.set_node_attributes(
        ...
        name = "IS",
        values = {
            k: (float(pathImportances[k]) / maximportance)
            for k in list(original.nodes())
        },
    )
    ...
    for code in codes:
        ...
        for i in range(1, 6):
            [
                ...
                open("pickles/" + code + "_" + str(i) + "_local1.pickle", "rb")
            ).load()
            model = modelHolder(storeModel3)
            pathVals.append(
                pickle.Unpickler(
                    open("pickles/" + code + "_" + str(i) + "_scores1.pickle", "rb")
                ).load()
            )
...
def scorePathway(RAs, pathImportances, CVdict):
    ...
    for i in range(1000):
        ...
        for node in pathImportances:
            tempscore += (
                abs(RAs[allNodes[randint(0, len(allNodes) - 1)]])
                * math.log(pathImportances[node], 2)
                * CVdict[node]
            )
