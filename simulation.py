...
class modelClass:
    def __init__(self, graph, sss, groundTruth):
        ...
        nodeDict = (
            {}
        )  # identifies names of nodes with their index in the node list- provide name, get index
        possibilityLister = []
        possibilityInverter = []
        predProbs = []
        predCorrs = []
        ...
        for j in range(
            0, len(sss)
        ):  # construct empty lists to stick values in later for intiial value list
        ...
        for i in range(0, len(nodeList)):
            ...
            if len(predtemp) > 3:
                if groundTruth:
                    while (
                        len(predtemp) > 3
                    ):  # handle case where there are too many predecessors by truncation
            ...
            withNones = list(zip(preds, itertool.repeat("empty")))
        ...
        self.nodeList = (
            nodeList  # define the node list simply as the nodes in the graph.
        )
    ...
    def update_upstream(self, node, newUpstreams):
        withNones = list(zip(newUpstreams, itertool.repeat("empty")))
        possibilities = list(itertool.product(*withNones))
        for j in range(0, len(possibilities)):
            possibilities[j] = list(possibilities[j])
            while "empty" in possibilities[j]:
...
class paramClass:
    def __init__(self,):
        ...
        self.crossoverProb = 0.6  # prob of crossing over a particular parent
        self.mutationProb = 0.4  # prob of mutating a particular parent
        self.rewire = False
        ...
        self.verbose = True
        self.bitFlipProb = 0.1  # prob of flipping bits inside mutation
        self.sigmaNetwork = 0
        self.sigmaNode = 0
        self.trials = 1
...
def runBool(
    individual, model, simSteps, initValues, params, knockouts, knockins, asynchronous
):
    if asynchronous:
        return asyncBool(
            individual, model, simSteps, initValues, params.iters, knockouts, knockins
        )
    else:
        ...
        for i in range(0, nodeNum):
            ...
            if i in knockouts:
                ...
                if sum(individual[model.individualParse[i] : end]) == 0:
                    ...
                    # temp=1
                    temp = updateBool(
                        i, oldValue, individual[model.individualParse[i] : end], model
                    )
            newValue[i] = temp
            simData[i, step] = temp
    avg = [
        0.1 * np.count_nonzero(simData[i, simSteps - 10 : simSteps])
...
def asyncBool(individual, model, simSteps, initValues, iters, knockouts, knockins):
    ...
    for i in range(0, params.iters):
        ...
        seq = list(range(0, len(model.nodeList)))
        # iterate over number of steps necessary
        for step in range(0, simSteps):
            oldValue = list(newValue)
            # shuffle- async
            ...
            for seq[i] in range(0, len(model.nodeList)):
                ...
                if seq[i] in knockouts:
                    ...
                    temp = updateBool(
                        seq[i],
                        newValue,
                        individual[model.individualParse[seq[i]] : end],
                        model,
                    )
                newValue[seq[i]] = temp
...
def NPsync(individual, model, cells, sampleProbs, params, KOs, KIs, syncBoolC):
    ...
    for j in range(0, cells):
        ...
        initValues = genEBNInitValues(
            individual, model, sampleProbs
        )  # get initial values for all nodes
        vals = np.zeros(
            len(model.nodeList), dtype=np.intc, order="C"
        )  # initiate output array
        initValues = np.array(initValues, dtype=np.intc, order="C")  # setup input array
        initValues1 = ctypes.c_void_p(
            initValues.ctypes.data
        ...
        cellArray.append(0.1 * np.copy(vals))
    return [((1.0 * np.sum(col)) / cells) for col in zip(*cellArray)]
...
def NPasync(individual, model, cells, sampleProbs, params, KOs, KIs):
    ...
    for j in range(0, cells):
        ...
        asyncResult = asyncBool(
            individual, model, simSteps, initValues, params.iters, KOs, KIs
...
def updateBool2(currentNode, oldValue, nodeIndividual, model):
    ...
    tempandNodeInvertList = [
        xi + [-1] * (3 - len(xi)) for xi in model.andNodeInvertList[currentNode]
    ]
    ...
    andLenList = np.array(
        [len(lister) for lister in model.andNodeList[currentNode]], dtype=np.intc
    )
