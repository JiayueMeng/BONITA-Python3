...
def Get_expanded_network(rules, equal_sign="*="):
  ...
  The expanded network for the given Boolean network model.
    ...
    for line in rules:
        ...
        if (
            update_rule[0] == "(" and update_rule[-1] == ")"
        ):  # remove parens from parent
    ...
    nx.set_node_attributes(
        G_expand,
        "Display Name",
        {k: " " if k in composite_nodes else k for k in list(G_expand.nodes())},
    )
    nx.set_node_attributes(
        G_expand,
        "andNode",
        {k: 1 if k in composite_nodes else 0 for k in list(G_expand.nodes())},
    )
...
def sortFpkms(
    data,
):  # puts fpkms data into appropriate lists of steady state dictionaries following normalization to largest value (as denominator)
    sss = []
    for j in range(1, len(data[1])):
        sss.append({})
    for i in range(1, len(data)):
...
def synthesizeInputs(
    graph, samples
):  # generates synthetic completely random inputs for simulation to steady state
    sss = []
...
def simTest():
    ...
    with open("testRules.txt") as csvfile:
        model, individual, graph = makeModelRules(
            csvfile.readlines(), sss=sampleList, equal_sign="="
        )
